...
std::vector<Movement> movements; // This will hold all movements
...
void moveCar(int inputValue)
{
  Serial.printf("Got value as %d\n", inputValue);
    ...
    if (inputValue != lastDirection) {
        unsigned long now = millis();
        if (lastDirection != STOP) {
            movements.push_back({lastDirection, lastStartTime, now - lastStartTime});
        }
        lastStartTime = now;
...
void traceBackMovements() {
    for (int i = movements.size() - 1; i >= 0; --i) {
        ...
        switch (movements[i].inputValue) {
            ...
            default: reverseDirection = STOP; break;
        }


        // Move in the reversed direction for the recorded duration
        moveCar(reverseDirection);
        delay(movements[i].duration); // Wait for the duration of the original movement
        moveCar(STOP); // Ensure the car stops before the next movement
    }
    movements.clear(); // Optionally clear the movement log after tracing back
}
